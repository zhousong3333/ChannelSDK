...
//        UnityPlayer.UnitySendMessage(UnityGameObject,method,message);
    ...
    @Override
    protected void Init(String message) //需要测试。 也可以传进来的时候传入json
    {
        JSONObject info = new JSONObject(message).getJSONObject("");
        info.getString("UnityGameObject");

        UnityGameObject =  param[0];
        String appId = param[1];
        String signKey = param[2];
    }
